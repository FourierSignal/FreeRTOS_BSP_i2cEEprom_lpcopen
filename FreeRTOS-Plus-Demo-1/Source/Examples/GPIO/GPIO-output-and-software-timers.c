...
*/
//
//lecture - explore these headers and add comments
//
/* FreeRTOS includes. */
#include "FreeRTOS.h"
#include "timers.h"
...
//
//lecture - explore FREERTOS+IO headers and add comments
//
...
//
//lecture - explore CMSIS firmware driver library headers and add comments
//
/* Library includes. */
...
//
//
//lecture - explore example header and add comments
//
//
/* Example includes. */
...
//
//
//lecture - important methods and variables associated with this
//          application - refer to header and source code
//
/*
...
static LPC_GPIO_T *GPIO_GetPointer(uint8_t portNum) ; //aaa-zzz


//
//lecture - many of the macros below are defined, in LPCXpresso17xx-base-board.h - refer to it
//          as needed - add more comments to header file(s)
//
...
//aaa-zzz
static LPC_GPIO_T *GPIO_GetPointer(uint8_t portNum)
{
	LPC_GPIO_T *pGPIO = NULL;

	switch (portNum) {
	case 0:
		pGPIO = (LPC_GPIO_T *)LPC_GPIO0_BASE;
		break;
	case 1:
		pGPIO = (LPC_GPIO_T *)LPC_GPIO1_BASE;
		break;
	case 2:
		pGPIO = (LPC_GPIO_T *)LPC_GPIO2_BASE;
		break;
	case 3:
		pGPIO = (LPC_GPIO_T *)LPC_GPIO3_BASE;
		break;
	case 4:
		pGPIO = (LPC_GPIO_T *)LPC_GPIO4_BASE;
		break;
	default:
		break;
	}
...
//
//
//lecture - many of the hw manipulations are done, using CMSIS core or CMSIS firmware driver library -
//          explore and add comments, as needed
//
//
...
/* Set the LED outputs to output. */
	...
	//	GPIO_SetDir( ucLEDPorts[ lx ], ulLEDBits[ lx ], boardGPIO_OUTPUT );
		...
		portNum = ucLEDPorts[ lx ];

		pGPIO= GPIO_GetPointer(portNum);  //aaa-zzz
		Chip_GPIO_SetDir(pGPIO,portNum,ulLEDBits[ lx ],boardGPIO_OUTPUT);
	...
	//
	//lecture - add more comments based on sw timer subsystem ?? extract comments from the respective
	//          header and source code - refer to freertos manuals and web-page
	//
	//
    ...
    //
    //lecture - specific rules must be understood - sw timer subsystem and scheduler
    //
    //
...
/*-----------------------------------------------------------*/
//
//lecture - we must first this logic and see how well it works - once done, we must
//          explore, add comments and possibly, modify as well
//
//
...
LPC_GPIO_T *pGPIO;
	...
	if( ucState == boardLED_ON )
	{
		...
		// GPIO_SetValue( ucLEDPorts[ ucLED ], ulLEDBits[ ucLED ] );
		 ...
		 portNum = ucLEDPorts[ ucLED ];

		 pGPIO= GPIO_GetPointer(portNum);  //aaa-zzz

		 Chip_GPIO_SetValue( pGPIO,portNum,ulLEDBits[ ucLED ] );
	}
	else
	{
		...
		Chip_GPIO_ClearValue( pGPIO,portNum,ulLEDBits[ ucLED ] );
