...
/*-----------------------------------------------------------*/
//
//lecture - i2c-eeprom - this api is invoked from corresponding ioctl call from
//          i2c-to-and-from-eeprom.c for changing the setting of tx transfer mode
//
//lecture - i2c-eeprom - vIOUtilsCreateTransferControlStructure() is invoked and
//                       present in FreeRTOS_IOUtils.c
//
//lecture - i2c-eeprom - old transfer state is deleted and a new tranfer state is created
//                       for zerocopytx state and relevant mutex semaphore is created
//
//lecture - UART - most points are similar to i2c case
//
//lecture - UART - invoked from  FreeRTOS_ioctl(), which is invoked from
//                 UART-interrupt-driven-command-console.c
//
